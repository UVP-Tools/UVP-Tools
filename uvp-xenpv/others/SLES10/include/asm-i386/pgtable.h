...
extern unsigned long long __PAGE_KERNEL, __PAGE_KERNEL_EXEC, __PAGE_KERNEL_PAGE_TABLE;
...
#define PAGE_KERNEL_PAGE_TABLE  __pgprot(__PAGE_KERNEL_PAGE_TABLE)
...
({									\
      ...
      if (pte_dirty(*(ptep)))                				\
              ...
              if (ret)                					\
                      pte_update_hook((vma)->vm_mm, (addr), (ptep));    \
      }                							\
      ret;                						\
})
...
{
	pte_t pte = _ptep_get_and_clear(ptep);
	pte_update_hook(mm, addr, ptep);
	return pte;
}
