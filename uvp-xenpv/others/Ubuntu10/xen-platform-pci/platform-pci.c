...
#include <xens/interface/hvm/params.h>
...
void  write_feature_flag()
{
	...
	rc = xenbus_write_uvp(XBT_NIL, "control/uvp", "feature_flag", str);
	if (rc) {
...
void  write_monitor_service_flag()
{
	...
	rc = xenbus_write_uvp(XBT_NIL, "control/uvp", "monitor-service-flag", "true");
	if (rc) {
...
void  write_driver_resume_flag()
{
	...
	rc = xenbus_write_uvp(XBT_NIL, "control/uvp", "driver-resume-flag", "1");
	if (rc) {
