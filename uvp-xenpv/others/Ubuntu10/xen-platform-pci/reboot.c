			     ...
			     const char **vec, unsigned int len)
{
 ...
 again:
	...
	str = (char *)xenbus_read_uvp(xbt, "control", "shutdown", NULL);
  ...
  /*将其清空后，如果热迁移时suspend失败则会导致迁移挂死，去掉后suspend失败后则会迁移超时*/
	//xenbus_write_uvp(xbt, "control", "shutdown", "");
			  ...
			  unsigned int len)
{
 ...
 again:
	...
	if (err)
		return;
	if (!xenbus_scanf_uvp(xbt, "control", "sysrq", "%c", &sysrq_key)) {
	...
	if (sysrq_key != '\0')
		xenbus_printf_uvp(xbt, "control", "sysrq", "%c", '\0');
...
static int setup_suspend_evtchn(void)
{
	...
	if (irq > 0)
		unbind_from_irqhandler_uvp(irq, NULL);
	...
	xenbus_write_uvp(XBT_NIL, "device/suspend", "event-channel", portstr);
...
static int setup_shutdown_watcher(void)
{
	...
	xenbus_scanf_uvp(XBT_NIL, "control",
		     "platform-feature-multiprocessor-suspend",
		     "%d", &fast_suspend);

	err = register_xenbus_watch_uvp(&shutdown_watch);
	if (err) {
		printk(KERN_ERR "Failed to set shutdown watcher\n");
		return err;
	}

	err = register_xenbus_watch_uvp(&sysrq_watch);
	if (err) {
