...
#include <linux/compat_virtionet.h>
#include <net/arp.h>
#include <linux/if_arp.h>

/* device supports hardware address
 * change when it's running */					 
static unsigned int iff_live_addr_change = 1; 
...
static int eth_prepare_mac_addr_change(struct net_device *dev, void *p)
{
	...
	if (!iff_live_addr_change && netif_running(dev))
		return -EBUSY;
...
static struct net_device_stats *virtnet_stats(struct net_device *dev)
{
	...
	for_each_possible_cpu(cpu) {
		struct virtnet_stats *stats_tmp = per_cpu_ptr(vi->stats, cpu);
		u64 tpackets, tbytes, rpackets, rbytes;
		
		
		do {
			start = u64_stats_fetch_begin_bh(&stats_tmp->tx_syncp);
			tpackets = stats_tmp->tx_packets;
			tbytes   = stats_tmp->tx_bytes;
		} while (u64_stats_fetch_retry_bh(&stats_tmp->tx_syncp, start));

		do {
			start = u64_stats_fetch_begin_bh(&stats_tmp->rx_syncp);
			rpackets = stats_tmp->rx_packets;
			rbytes   = stats_tmp->rx_bytes;
		} while (u64_stats_fetch_retry_bh(&stats_tmp->rx_syncp, start));
...
/* from higher version kernel code. For redhat*/
...
static int virtnet_probe(struct virtio_device *vdev)
{
...
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,24))	
	...
	if (virtio_has_feature(vdev, VIRTIO_NET_F_CSUM)) {
		...
		if (virtio_has_feature(vdev, VIRTIO_NET_F_GSO)) {
			dev->features |= NETIF_F_TSO | NETIF_F_TSO_ECN | NETIF_F_TSO6;
		}
	...
	get_online_cpus();
	err = virtnet_set_queues(vi, max_queue_pairs);
	if (!err) {	
		netif_set_real_num_tx_queues(dev, max_queue_pairs);
		netif_set_real_num_rx_queues(dev, max_queue_pairs);
